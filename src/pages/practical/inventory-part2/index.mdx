...
a simple annotation on the ingress to enable integration with AppId. The starter kits have already been
...
- Copy the json contents from the credentials into `mappings.json` under the `CLOUDANT_CONFIG` object (keep in mind the CLOUDANT_CONFIG value must be a string type not a json type, so you must use escaping characters for this value.
    ```json path=src/main/resources/mappings.json
    {
      "DATABASE_NAME": "inventory-{namespace}",
